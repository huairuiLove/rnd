...
> 2. 第 9.1 节以**分块对角**（在标签条件独立前提下精确）取代 K-FAC + rank-$r$ 截断——在单 sigmoid
>    头设定下这层近似是多余的，不是"更省"而是"本不需要"（式 14/14'）。两段式筛选**保留**，但粗筛
>    换成可容许上界式 (16)（初稿称"全池直接精算即可"已在 9.1/9.4 更正）；
> 3. 第 12 节查询算法据此重写，并显式给出阻尼 $\delta=2\,\mathrm{wd}\,N/d$ 的推导来源与
>    `GV_eff` 的定位（本版已撤回「$\delta$ 非手调、与拟合目标严格一致」的说法：实时模型用 AdamW，
>    $\delta$ 只是调参旋钮；见第 12 节）；
...
（旧版此处写式 (9)；式 (9) 亦是上界，但式 (16) 更紧，见 9.4），再对工作集用
...
**这是本次 CoMAL 适配相对原方案的主要简化；在第 2 节的结构前提（单 sigmoid 头 + 标签条件独立的
对数似然）下它是精确的而非近似的。**「精确」限定于该前提：一旦按第 6 节引入经验标签协方差
$\Sigma$，跨标签块即出现，分块对角随之失效（见第 6 节代价表与 9.1 末的边界说明）。
...
**但两段式筛选必须保留（本版更正）。** 初稿据式 (14') 是闭式便称「全池直接精算即可」，这一步是错的：
全池精算本身需 $O(|U|Cd)$ 次内积，且 $m_c(x)=z^\top A_c^{-1}z$ 依赖 $C$ 个 $d\times d$ 逆——在
$|U|=2\times10^4$、$C=54$、$d=769$ 下这不是可忽略项。真正不必要的是初稿那种用**不可容许**启发式
的粗筛；本版改为用**可容许上界**式 (16) 粗筛（$\Delta_{\mathrm{ub}}\ge\Delta$，故被弃候选可给出
池级证书），再对工作集精算式 (14')。式 (9) 仅作为消融项保留（检验推论 2.1 的饱和效应是否在起作用）。
...
3. **复杂度（第 6--8 步）.** 三个独立的坑：(i) 全池精算式 (14')；(ii) 每步重做 Cholesky；
   ...
   改为上界筛选 + Sherman-Morrison + 缓存 m 后，历史基准为 11.8s（约 48 倍）；当前环境的
   `rnd_src/_check_cost.py` 实测为 20.2s，具体耗时随 BLAS、线程数和硬件变化，不能跨机器直接比较，
...
| 硬伪标签（BADGE 式）替代标签期望 | 标签边缘化相对单点伪标签是否有增益？（**不**预设 BADGE 是「补偿」，见第 3 节更正） | 命题 1、第 3 节 |
